...
class CtrLoRA:
    def __init__(self, num_loras=1):
        ...
        if num_loras == 1:
            self.config_file = 'configs/inference/ctrlora_taesd1.yaml'
            #self.config_file = 'configs/inference/ctrlora_sd15_rank128_1lora.yaml'
        elif num_loras == 2:
            self.config_file = 'configs/inference/ctrlora_taesd2.yaml'
        else:
    ...
    def create_model(
        ...
        print(f"[Model Initialization] 실행 시간: {init_elapsed:.4f} sec")
    ...
    def is_conventional_lora_key(self, k):
        return 'lora_down' in k or 'lora_up' in k
    
    def load_conventional_lora(self, conv_lora_file):
        """
        conventional LoRA 파일을 불러와서 기존 stable diffusion 모델에 적용합니다.
        conv_lora_file: 보편적인 LoRA 체크포인트 파일의 경로
        """
        assert os.path.exists(conv_lora_file), f'File not found: {conv_lora_file}'
        conv_lora_state_dict = load_state_dict(conv_lora_file, location='cpu')
        # conventional LoRA의 경우 보통 'lora_down'과 'lora_up' 키를 사용합니다.
        conv_lora_state_dict = {k: v for k, v in conv_lora_state_dict.items() if self.is_conventional_lora_key(k)}
        self.model.load_state_dict(conv_lora_state_dict, strict=False)
        del conv_lora_state_dict
