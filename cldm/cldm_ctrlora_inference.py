...
class ControlInferenceLDM(ControlLDM):
    ...
    def apply_model(self, x_noisy, t, conds, *args, **kwargs):
        ...
        for i, cond in enumerate(conds):
            ...
            # 타이머 추가: ControlNet 실행 시간 측정
            start_control = time.time()
            control = self.control_model(hint=hint, timesteps=t, context=cond_txt)
            control_elapsed = time.time() - start_control
            #print(f"[ControlNet] 실행 시간: {control_elapsed:.4f} sec")
        ...
        # 여기서 U-Net (diffusion_model)의 실행 시간을 측정함
        start = time.time()   # 타이머 시작
        eps = diffusion_model(x=x_noisy, timesteps=t, context=cond_txt, control=control, only_mid_control=self.only_mid_control)
        elapsed = time.time() - start
        #print(f"[U-Net] 실행 시간: {elapsed:.4f} sec")
        return eps
